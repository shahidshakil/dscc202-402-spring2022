...
string_columns=airbnbDF.select_dtypes(include=['object']).astype('category').columns
df = pd.get_dummies(airbnbDF,drop_first=True, columns = string_columns)
...
for c in df.columns:
    ...
    print("There are no Columns with object datatype")
...
# MAGIC %md
# MAGIC ##### There are no NaNs in any of the columns so there is no need to perform any further preprocessing and neither is any imputation required.
# MAGIC ##### So we will go ahead and fit a model
...
rnf = RandomForestRegressor(random_state = 42)
rnf.fit(X_train, y_train);
predicted_values = rnf.predict(X_test)
...
errors = abs(predicted_values - y_test)
...
with mlflow.start_run(run_name = "iteration_1") as run:
    rf = RandomForestRegressor()
    rf.fit(X_train, y_train)
    predictions = rf.predict(X_test)
    mlflow.sklearn.log_model(rnf, "rf_model_pipeline")
...
logged_model = 'runs:/833b0414b89f4d1bacf5adc205137e1c/random-forest-model'
...
# TODO
final_model_path =  f"{working_path}/final-model_1"
...
def model_predict(final_model_path, test_data_path, prediction_path):
    # FILL_IN
    loaded_model = mlflow.pyfunc.load_model(final_model_path)
    model_output = loaded_model.predict(pd.read_csv(test_data_path))
    model_output.to_csv(prediction_path,index=False)
...
# TODO
second_prediction_path = f"{working_path}/predictions.csv"
mlflow.projects.run(working_path,parameters={
